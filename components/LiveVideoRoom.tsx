...
export default function LiveVideoRoom({
  ...
  const updateRoomParticipantLifecycle = useCallback(async () => {
    try {
      await fetch(`/api/study-rooms/${encodeURIComponent(roomId)}/leave`, {
        method: "POST",
        keepalive: true,
      });
    } catch (error) {
      console.error("Study Room Leave Error:", error);
    }
  }, [roomId]);
